...
async function main() {
  const customerId = 1; // 新创建的客户

  console.log(`Seeding contacts for customer #${customerId}...`);
  ...
  if (existingContacts.length > 0) {
    console.log(`Contacts already exist: ${existingContacts.length}`);
    ...
    for (const contact of contacts) {
      ...
      contactIds[contact.name] = last_id;
...
只需要公司名称和您的手机号就行，我这边帮您注册好。`,
  ...
  for (const sample of sampleChats) {
    const contactId = contactIds[sample.contactName];
    ...
    if (chatCount[0].count > 0) {
      console.log(`${sample.contactName} 已有聊天记录，跳过`);
      continue;
    }
    ...
    await db.execute({
      sql: `INSERT INTO wechat_chats (customer_id, wechat_contact_id, raw_content, chat_date, analysis_status)
            VALUES (?, ?, ?, ?, 'completed')`,
      args: [customerId, contactId, sample.content, sample.chatDate],
    });
    console.log(`为 ${sample.contactName} 添加了聊天记录`);
  }
