...
function getBaseURL(apiKey: string): string {
  // API2D relay service
  if (apiKey.startsWith('fk')) return 'https://oa.api2d.net/v1';
  // Default OpenAI
...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey) {
    return NextResponse.json({ error: '未配置 API Key，请在设置页面添加' }, { status: 400 });
  }
  ...
  try {
    ...
    if (msg.includes('401')) friendly = 'API Key 无效，请在设置中检查';
    else if (msg.includes('429')) friendly = '请求频率超限，请稍后重试';
