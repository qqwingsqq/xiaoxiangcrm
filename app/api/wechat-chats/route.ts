  ...
  const { rows } = await db.execute({
    sql: `
      SELECT
        ...
        COALESCE((SELECT MAX(w5.last_message_at) FROM wechat_chats w5 WHERE w5.customer_id = wc.customer_id), wc.created_at) AS latest_message_at
      FROM wechat_chats wc
      JOIN customers c ON c.id = wc.customer_id
      WHERE (c.is_blocked = 0 OR c.is_blocked IS NULL)
        ...
        AND wc.id = (
          ...
          ORDER BY w4.last_message_at DESC, w4.created_at DESC
          LIMIT 1
        )
